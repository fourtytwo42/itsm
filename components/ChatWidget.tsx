...
export default function ChatWidget() {
  ...
  const handleSend = async () => {
    if (!input.trim() || loading) {
      console.log('[ChatWidget] handleSend: Skipping - input empty or already loading', { input: input.trim(), loading })
      return
    }
    ...
    console.log('[ChatWidget] handleSend: Starting', { 
    ...
    try {
      ...
      console.log('[ChatWidget] handleSend: Sending request', { 
      ...
      const timeoutId = setTimeout(() => {
        console.error('[ChatWidget] handleSend: Request timeout after 60s')
        controller.abort()
      ...
      console.log('[ChatWidget] handleSend: Response received', { 
      ...
      if (!response.ok) {
        ...
        console.error('[ChatWidget] handleSend: HTTP Error', { 
          status: response.status, 
      ...
      console.log('[ChatWidget] handleSend: Response parsed', { 
      ...
      if (!data.success) {
        console.error('[ChatWidget] handleSend: API Error Response', { 
          error: data.error,
      ...
      if (!data.data?.reply) {
        console.error('[ChatWidget] handleSend: No reply in response', { data })
        throw new Error('No reply received from server')
      }

      // Update conversationId if returned
      if (data.data.conversationId) {
        console.log('[ChatWidget] handleSend: Conversation ID updated', { 
      ...
      console.log('[ChatWidget] handleSend: Adding assistant message', { 
        messageLength: assistantMessage.content.length 
      })
      
      setMessages((prev) => {
        const updated = [...prev, assistantMessage]
        console.log('[ChatWidget] handleSend: Messages updated', { 
          totalMessages: updated.length 
        })
        return updated
      })
    } catch (error) {
      console.error('[ChatWidget] handleSend: Exception caught', { 
        error,
      ...
      console.log('[ChatWidget] handleSend: Finalizing (setting loading to false)')
      setLoading(false)
    }
  }
  ...
  const handleClearChat = () => {
    ...
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem(CHAT_STORAGE_KEY, JSON.stringify(initialMessages))
      } catch (error) {
