...
export default function ManagerAgentsPage() {
  ...
  const loadUsers = async () => {
    try {
      ...
      const response = await fetch(`/api/v1/manager/agents?${params.toString()}`, {
      ...
      if (data.success) {
        ...
        // Handle both data.users (from manager/agents) and data.data.users (from admin/users)
  ...
  const handleSaveTenantAssignments = async () => {
    ...
    try {
      const token = localStorage.getItem('accessToken')
      const currentAssignments = userTenantMap[selectedUser.id] || []
      const currentTenantIds = new Set(currentAssignments.map(a => a.tenantId))

      const assignmentsToAdd = selectedTenantIds.filter(id => !currentTenantIds.has(id))
      const assignmentsToRemove = currentAssignments.filter(a => !selectedTenantIds.includes(a.tenantId))

      // Use admin endpoint for IT_MANAGER users, manager endpoint for others
      const baseEndpoint = selectedUser.role === RoleName.IT_MANAGER
        ? `/api/v1/admin/users/${selectedUser.id}/tenants`
        : `/api/v1/manager/users/${selectedUser.id}/tenants`

      await Promise.all([
        ...assignmentsToAdd.map(tenantId =>
          fetch(baseEndpoint, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        ...assignmentsToRemove.map(assignment =>
          fetch(`${baseEndpoint}?assignmentId=${assignment.id}`, {
            method: 'DELETE',
            headers: {
              Authorization: `Bearer ${token}`,
            },
